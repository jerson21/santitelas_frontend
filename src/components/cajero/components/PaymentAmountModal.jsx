...
const PaymentAmountModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && inputRef.current) {
      ...
      setMontoRecibido('');  // Limpiar el monto al abrir el modal
  ...
  useEffect(() => {
    ...
    // Redondear el vuelto a pesos completos (sin centavos)
    const vueltoRedondeado = Math.round(cambio);
    setVuelto(vueltoRedondeado > 0 ? vueltoRedondeado : 0);
